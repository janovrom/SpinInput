...
namespace UnityEngine.UI
{
    ...
    [AddComponentMenu("UI/Spiral Input Field", 31)]
    public class SpiralInputField
        : Selectable,
        ...
        ICanvasElement
    {
        ...
        public GameObject spiralInput;

        private bool _activateAtEndOfFrame = false;
        ...
        private void UpdateGeometry()
        {
            ...
            OnFillVBO(m_Vbo);
            Mesh m = new Mesh();
            m.vertices = m_Vbo.Select(x => x.position).ToArray();
            m.SetUVs(0, m_Vbo.Select(x => x.uv0).ToList());
            m.SetUVs(1, m_Vbo.Select(x => x.uv1).ToList());
            m.SetUVs(2, m_Vbo.Select(x => x.uv2).ToList());
            m.SetUVs(3, m_Vbo.Select(x => x.uv3).ToList());
            m.SetTangents(m_Vbo.Select(x => x.tangent).ToList());
            m.SetNormals(m_Vbo.Select(x => x.normal).ToList());
            m.SetColors(m_Vbo.Select(x => x.color).ToList());
            m.SetIndices(m_Vbo.Count > 0 ? new int[] { 0,2,1,0,3,2 } : new int[] { }, MeshTopology.Triangles, 0);
            m_CachedInputRenderer.SetMesh(m);
            //if (m_Vbo.Count == 0)
        ...
        public override void OnSelect(BaseEventData eventData)
        {
            ...
            ActivateInputField();
            SpiralInputEventSystem.current.InputFieldSelected(this);
            spiralInput.SetActive(true);
        }
        ...
        public override void OnDeselect(BaseEventData eventData)
        {
            ...
            {
                if ((eventData as PointerEventData).pointerCurrentRaycast.gameObject.CompareTag("VirtualButton"))
                {
                    deactivate = false;
                    _activateAtEndOfFrame = true;
        ...
        private void Deactivate(BaseEventData eventData)
        {
            ...
            SpiralInputEventSystem.current.InputFieldDeselected(this);
            spiralInput.SetActive(false);
        }
        ...
        public void ProcessVirtualKey(Event e)
        {
            bool consumedEvent = false;
            _activateAtEndOfFrame = true;
